...
- `src/town.js` owns terrain, roads, more detailed low-poly building models, destructible town props, staged structural damage, pressure bursts, rotating per-frame simulation budgets, frame-budgeted rubble/effect spawning, persistent capped ground scars, wider 5x5 procedural town chunk loading, spatial buckets for nearby destructible simulation, distance-based render LOD, lift thresholds, suction response, and destroyed ratio.
...
- Change levels, category-scaled score/damage targets, queued absorption pacing, timer, combo, pause behavior, camera angle/zoom/fog composition, post-processing setup, scene debris caps, minimum level duration, or diagnostics in `src/game.js`.
...
- Procedural town chunks are generated near the current edge and stay loaded; town object updates, huge-radius candidate simulation, rubble effects, scene debris, and distant details/props are budgeted/capped, but full chunk unloading or instancing is still the next big lever if long runs get dense.
